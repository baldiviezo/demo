...
function readHistorical() {
    ...
    formData.append('readHistorical','');
    fetch('../historical', {
        method: "POST",
        body: formData
...
function paginacionCustomer(allCustomers, page){
    ...
    let liActive;
    if(page > 1){
        li+= `<li class="btn" onclick="paginacionCustomer(${allCustomers}, ${page-1})"><img src="../img/arowLeft.svg"></li>`;
    }
    ...
    if(page < allPages){
        li += `<li class="btn" onclick="paginacionCustomer(${allCustomers}, ${page+1})"><img src="../img/arowRight.svg"></li>`;
    }
    ul.innerHTML = li;
        ...
        for(let valor in filterLogo[customer]){
            ...
            if(valor == 'id_avi'){
                ...
                if(filterLogo[customer][valor] == 'Evento' ){
                    //tr.setAttribute('style', 'color: #00e81b44; background-color: var(--color-font-title)');
                }else if(filterLogo[customer][valor] == 'Advertencia' ){
                    //tr.setAttribute('style', 'color: #f3ff57; background-color: var(--color-font-title)');
                }else if (filterLogo[customer][valor] == 'Error' ){
                    //tr.setAttribute('style', 'color: #f008; background-color: var(--color-font-title)');
                }       
                td.innerText = filterLogo[customer][valor];
                tr.appendChild(td);
